...
def write_in_log(message):
    global repertory
    log_file = open(os.path.join(repertory, "log/log_handle.txt"), "a")
    try:
...
def fetch_backup_list(chemin_principal, date_now, password, config_json):
    ...
    for elt in os.listdir(chemin_principal):
        if os.path.isdir(os.path.join(chemin_principal, elt)):
            folders = os.path.join(chemin_principal, elt)
            for paths, dirs, files in os.walk(folders):
                for f in files:
                    chemin_second = os.path.join(folders, f)
                    files_time = dt.datetime.fromtimestamp(os.path.getctime(chemin_second)).strftime('%Y-%m-%d')
                    if files_time == date_h_now:
                        ...
                        if state:
                            ...
                            filename = os.path.join(chemin_second, f)
                            file = open( filename, 'rb')
                            try:
                                write_in_log("{} DONE: File {} created successfully!!\n".format(date_h_now, f))
                                email_text +=  "{} DONE: File \" {} \" created successfully!!\n      ".format(date_h_now, f)
                                print("{} DONE: File {} created successfully!!".format(date_h_now, f))
                            except Exception as e:
                                print("{} ERROR: {}!!! File not created !!!!".format(date_h_now, e))
                                write_in_log("{} ERROR: {}!!! File not created !!!!\n".format(date_h_now, e))
                                email_text +=  "{} FAILED: File \" {} \" failed to check!!\n      ".format(date_h_now, f)
                            file.close()
                            session.quit()
        else:
            ...
            if file_time == date_h_now:
                ...
                if state:
                    ...
                    try:
                        ...
                        email_text +=  "{} DONE: File \" {} \" created successfully!!\n      ".format(date_h_now, elt)
                        print("{} DONE: File {} created successfully!!".format(date_h_now, elt))
                    except Exception as e:
                        print("{} ERROR: {}!!! File not created !!!!".format(date_h_now, e))
                        write_in_log("{} ERROR: {}!!! File not created !!!!\n".format(date_h_now, e))
                        email_text +=  "{} FAILED: File \" {} \" failed to check!!\n      ".format(date_h_now, elt)
...
def list_backup():
    ...
    for elt in os.listdir(config_json['source']):
        if os.path.isdir(os.path.join(config_json['source'], elt)):
            folders = os.path.join(config_json['source'], elt)
            for paths, dirs, files in os.walk(folders):
                for f in files:
                    chemin_second = os.path.join(folders, f)
                    files_time = dt.datetime.fromtimestamp(os.path.getctime(chemin_second)).strftime('%Y-%m-%d')
